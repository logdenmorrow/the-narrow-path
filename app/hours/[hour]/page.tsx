...
type RouteParams = Promise<{ hour: string }>;
...
function normalizeTaskTemplate(
  relation: HourTaskRow["task_templates"]
): { title: string; slug: string } {
  const template = Array.isArray(relation) ? relation[0] : relation;

  return {
    title: template?.title ?? "Liturgy of the Hours",
    slug: template?.slug ?? "",
  };
}
...
export default async function LiturgicalHourPage({
  params,
  searchParams,
}: {
  params: RouteParams;
  ...
  if (!isValidHour(resolvedParams.hour)) {
    redirect(`/hours/${getDefaultHour()}`);
  }

  const hour: LiturgicalHour = resolvedParams.hour;
  const hourDisplayName = getHourDisplayName(hour);
  const hourKicker = getHourKicker(hour);
  ...
  const { data: completionData } = hourTask
    ? await supabase
        .from("user_task_completions")
        .select("id")
        .eq("user_id", user.id)
        .eq("plan_day_task_id", hourTask.id)
        .maybeSingle()
    : { data: null };
  ...
  const title = nightPrayer?.title?.trim() || hourDisplayName;
  const subtitle =
    ...
    <main className="monastic-page">
      <PageFrame className="max-w-6xl space-y-5 sm:space-y-6">
        ...
        <AppActionBar
          ...
          actions={[
            {
              href: `/hours/${hour}?day=${previousDay}`,
              label: "Previous Day",
              variant: "outline",
            },
            {
              href: `/hours/${hour}?day=${challenge.currentDayNumber}`,
              label: "Jump to Current Day",
              variant: "secondary",
            },
            {
              href: `/hours/${hour}?day=${nextDay}`,
              label: "Next Day",
              variant: "outline",
            },
          ]}
        ...
        <SurfaceCard className={PAGE_CARD_CLASS}>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-monastic-0 sm:text-4xl">
              {hasImportedContent
                ? `Pray ${hourDisplayName}`
                : `${hourDisplayName} has not been imported for this date yet.`}
            </h2>
        ...
        {hourTask ? (
          <SurfaceCard className={PAGE_CARD_CLASS}>
            <SectionHeader
              ...
              description={`Mark this complete after praying ${hourKicker}.`}
            />
            <div className="mt-5">
              <TodayTaskCard
                planDayTaskId={hourTask.id}
                title={normalizeTaskTemplate(hourTask.task_templates).title}
                note={hourTask.requirement_note}
                isRequired={false}
                isOptional
