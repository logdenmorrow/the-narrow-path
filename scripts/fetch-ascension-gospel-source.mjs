...
const BOOKS = {
  Matthew: { chapters: 28, outputSlug: "matthew", routeBook: "Matt" },
  Mark: { chapters: 16, outputSlug: "mark", routeBook: "Mark" },
  Luke: { chapters: 24, outputSlug: "luke", routeBook: "Luke" },
  John: { chapters: 21, outputSlug: "john", routeBook: "John" },
};

const OUTPUT_DIR = "content/gospels/source";
const DIAGNOSTICS_OUTPUT = `${OUTPUT_DIR}/ascension-gospels-diagnostics.md`;
...
function extractChapter({ book, chapter, sourceUrl, html }) {
  ...
  for (let expected = 1; expected <= maxVerseNumber; expected += 1) {
    if (!finalVerseNumbers.has(expected)) {
      omittedVerseNumbers.push(expected);
    }
  }
  ...
  for (const verse of verses) {
    if (duplicateVerseNumbers.has(verse.verseNumber) && !verse.text) {
      warnings.push(`Resolved duplicate verse ${book} ${chapter}:${verse.verseNumber} was empty.`);
    }
  }
...
async function fetchChapter({ book, chapter }) {
  ...
  const response = await fetch(sourceUrl, {
    headers: {
      ...
      "user-agent": "The Narrow Path Gospel source review script",
    },
  });
...
function buildBookMarkdown(data) {
  ...
  if (data.diagnostics.artifacts.length > 0) {
    ...
    for (const artifact of data.diagnostics.artifacts) {
      lines.push(`- ${artifact}`);
    }
  }
...
function buildCombinedDiagnostics(results) {
  ...
  for (const result of results) {
    ...
    if (result.diagnostics.artifacts.length > 0) {
      lines.push("", `### ${result.book} Source Artifacts`, "");
    ...
    if (result.diagnostics.warnings.length > 0) {
      lines.push("", `### ${result.book} Warnings`, "");
  ...
  const allSafe = results.every((result) => result.diagnostics.safeForLaterReview);
  lines.push("", "## Expansion Assessment", "");
  lines.push(
    allSafe
      ...
      : "The extraction should not be used for later import-preparation work until the warnings above are resolved or explicitly accepted."
  );
  lines.push("");
  lines.push("Do not generate SQL, migrations, Supabase updates, or import-ready plan data directly from these files without a later explicit review step.");
...
async function fetchBook(book) {
  ...
  for (let chapter = 1; chapter <= config.chapters; chapter += 1) {
    ...
    if (extracted.verses.length === 0 && /sign in|log in|subscribe/i.test(fetched.html)) {
      extracted.warnings.push(
        "No verses were extracted, and the page appears to include sign-in/subscription language."
      );
    }
  ...
  const data = {
    ...
    source: {
      ...
      translation: "RSV2CE Gospel text source approved by Logan for review",
